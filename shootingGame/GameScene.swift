...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        _ = Timer.scheduledTimer(timeInterval: 0.3, target: self, selector: #selector(self.shootBullets), userInfo: nil, repeats: true) //increase the amount of time the bullets will come out of the rocket ship.
            //this is the timer for the bullets
        
        _ = Timer.scheduledTimer(timeInterval: 0.5, target: self, selector: #selector(self.shootAlienEnemies), userInfo: nil, repeats: true) //increase more enemies at any certain time, decrease time intervale < 1.0
            //this is the timer for the enemies that are falling
    ...
    func shootAlienEnemies(){
        ...
        let fallFromSky = SKAction.moveTo(y: -self.frame.size.height, duration: 5.0) //for the levels, we can decrease the duration to make it faster
